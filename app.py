...
import numpy as np
from predict import predict_am, predict_bmi, predict_alcohol, predict_expenditure
...
am=st.number_input('Enter adult mortality rate of your region', min_value=0, max_value=100, value=0, step=1)
...
bmi=st.slider('Enter your body mass index', min_value=0, max_value=100, value=0, step=1)
...
st.markdown(text, unsafe_allow_html=True)
st.text("Average total expenditure is the average amount spent by a person on health per year in USD in scale of1-10.")
...
if st.button('Predict'):
    ...
    print(expenditure)
    life=(am+bmi+alcohol+expenditure)/4
